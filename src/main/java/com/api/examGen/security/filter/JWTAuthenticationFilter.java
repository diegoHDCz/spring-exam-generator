package com.api.examGen.security.filter;
...
import static com.api.examGen.security.filter.Constants.*;
...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        String tokenJson = "{\"token\":"+addQuotos(token) + ",\"exp\":"+addQuotos(expTimeUTC.toString()) +"}";
        response.getWriter().write((tokenJson));
        response.addHeader("Content-Type","application/json;charset=UTF-8");
            response.addHeader(HEADER_STRING,token);
    }

    private String addQuotos(String value) {
